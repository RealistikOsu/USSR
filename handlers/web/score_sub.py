...
from helpers.user import get_achievements
from helpers.user import increment_playtime
from helpers.user import unlock_achievement
...
async def score_submit_handler(req: Request) -> Response:
    ...
    dupe_check = (
        await connections.sql.fetchcol(  # Try to fetch as much similar score as we can.
    ...
    await s.submit(restricted=privs & Privileges.USER_PUBLIC == 0)
    ...
    if s.completed == Completed.BEST and (
        s.bmap.status in (Status.RANKED, Status.QUALIFIED)
        and await surpassed_cap_restrict(s)
    ):
        await edit_user(
            Actions.RESTRICT,
            s.user_id,
    ...
    score = s.__dict__
    (c_rank, country) = await stats.c_rank()
    score |= {"user": {"rank": stats.rank, "c_rank": c_rank, "country": country}}

    score_data = {
        "type": "write",
        "write_key": config.WS_WRITE_KEY,
        "data": score,
    }

    if s.passed or s.score > 10000 and (not privs & Privileges.USER_PUBLIC == 0):
        try:
            async with websockets.connect(
                "ws://localhost:2137/ws/scores/feed",
            ) as websocket:
                await websocket.send(orjson.dumps(score_data))
        except Exception as e:
            error(
                f"Couldn't distribute the information to websocket! Error: {traceback.format_exc()} (Skipping...)",
            )
            pass
