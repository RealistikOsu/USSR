...
class PPChangeResult:
    ...
    async def from_score(cls, score: Score) -> "PPChangeResult":
        ...
        return cls(score, old_pp, new_pp)
...
TABLE_HEADERS = ("Score", "User", "Song Name", "PP")
DIVIDER = "-" * 15
...
class PPChangeCalc:
    ...
    async def load_score_diff(self) -> None:
        ...
        self.new_total_pp = round(
            calc_weighed_pp([s.score.pp for s in self.score_diff]),
            2,
        )
...
def parse_args() -> dict:
    ...
    try:
        ...
        invalid_args_err(
            f"Expected 3 command arguments to be supplied (received {arg_count})",
        )
