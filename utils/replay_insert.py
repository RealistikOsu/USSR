...
async def insert_replay_data(replay_path: str):
    ...
    s = Score(
        0, bmap, user_id,
        replay.score, replay.max_combo,
        replay.perfect, True, False, mods,
        c_mode, replay.n300, replay.n100,
        replay.n50, replay.nkatu, replay.ngeki,
        replay.nmiss, get_timestamp(), mode,
        None, 0.0, 0.0, 0, # TODO: Playtime
        0, "", 0, replay.player_name
    )
    s.calc_accuracy()
    ...
    r = (BinaryWriter()
        ...
        .write_u8_le(0 if not replay.perfect else 1)
    ...
    lzma_off = int.from_bytes(data[current_off:current_off+4], "little", signed= True)
...
def parse_args() -> dict:
    ...
    try:
        ...
        invalid_args_err(f"Expected 1 command arguments to be supplied (received {arg_count})")
...
def main():
    ...
    loop.run_until_complete(
        insert_replay_data(**data_parsed)
    )
