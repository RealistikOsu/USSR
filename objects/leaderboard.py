...
from globals.caches import add_nocheck_md5
...
async def _try_bmap(md5: str) -> tuple[FetchStatus, Optional[Beatmap]]:
    ...
    if res_bmap and res_bmap.deserves_update:
        await res_bmap.try_update()
...
class GlobalLeaderboard:
    ...
    _scores: dict[
        int,
        tuple[object, ...],
    ]  # TODO: Maybe look into making this a score object?
    ...
    async def __set_data_from_sql(self) -> None:
        ...
        for idx, score in enumerate(scores_db):
            ...
            if idx + 1 < SIZE_LIMIT:
                self._scores[score[USER_ID_IDX]] = score
            # Store all user_ids
    ...
    async def refresh(self) -> None:
        ...
        if self.bmap.has_leaderboard:
            await self.__set_data_from_sql()
    ...
    def insert_user_score(self, s: Score) -> None:
        ...
        if self.user_has_score(s.user_id):
            self.remove_user_score(s.user_id)
        ...
        if self.has_scores:
            ...
            place_idx = 0  # They have first place

        # Last place
        if place_idx == -1 and self.total_scores < SIZE_LIMIT:
            place_idx = self.total_scores

        # Score is not in leaderboard top. Ignore.
        if place_idx == -1:
            return
        score_dict = {
            i: self._scores[i] for i in tuple(self._scores.keys())[:place_idx]
        }
    ...
    async def get_user_pb(
        self,
        user_id: int,
        cache: bool = True,
    ) -> tuple[FetchStatus, Optional[PersonalBestResult]]:
        ...
        if not self.user_has_score(user_id):
            return FetchStatus.NONE, None
        if pb := self._pb_cache.get(user_id):
            return FetchStatus.CACHE, pb
...
class CountryLeaderboard(GlobalLeaderboard):
    ...
    @classmethod
    def from_cache(_, _1, _2, _3) -> None:
        return
