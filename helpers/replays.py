...
def build_full_replay(s: Score) -> Optional[BinaryWriter]:
    ...
    replay = BinaryWriter()
    replay.write_u8_le(s.mode.value)
    replay.write_i32_le(OSU_VERSION)
    replay.write_osu_string(s.bmap.md5)
    replay.write_osu_string(s.username)
    replay.write_osu_string(replay_md5)
    replay.write_i16_le(s.count_300)
    replay.write_i16_le(s.count_100)
    replay.write_i16_le(s.count_50)
    replay.write_i16_le(s.count_geki)
    replay.write_i16_le(s.count_katu)
    replay.write_i16_le(s.count_miss)
    replay.write_i32_le(s.score)
    replay.write_i16_le(s.max_combo)
    replay.write_u8_le(s.full_combo)
    replay.write_i32_le(s.mods.value)
    replay.write_u8_le(0)
    replay.write_i64_le(ts_to_utc_ticks(s.timestamp)) # FIXME: This results in incorrect dates.
    replay.write_i32_le(len(rp))
    replay.write_raw(rp)
    replay.write_i64_le(s.id)
    return replay
