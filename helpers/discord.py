...
async def log_user_edit(
    ...
    embed.description = (
        f"{username} ({user_id}) has just been {action.log_action}" f" for {reason}!"
    )
...
async def log_first_place(s: "Score", old_stats: "Stats", new_stats: "Stats") -> None:
    ...
    embed.add_field(
        ...
        value=f"[__[Download Map]({config.SRV_URL}/d/{s.bmap.set_id})__]",
    )
    embed.add_field(
        name=f"Played by: {s.username}",
        value=f"[__[User Profile]({config.SRV_URL}/u/{s.user_id})__]",
    )

    embed.set_image(
        url=f"https://assets.ppy.sh/beatmaps/{s.bmap.set_id}/covers/cover.jpg",
    )
